...
const btnMsg = computed(() =>
  activeAppId.value === 0 ? '创建我的个人应用' : '更新我的个人应用'
);
const title = computed(() =>
  activeAppId.value === 0 ? '创建专属应用' : '更新个人应用'
);

const mineApps = computed(() => {
  if (!keywords.value) return appCatStore.mineApps;
  else
    return appCatStore.mineApps.filter((item) =>
      item.appName.includes(keywords.value)
    );
});
...
<template>
  <div class="w-full flex justify-center">
    <div class="p-6 max-w-screen-4xl px-4 w-full">
      <div
        class="flex flex-col space-y-3 justify-between sm:flex-row sm:space-y-0"
      >
      ...
      >
        ...
        >
          <div class="w-full h-16 flex items-center mb-3">
            ...
            <span
              class="text-base font-bold mb-1 text-base text-[#333] dark:text-[#ffffff85]"
              >{{ item.appName }}</span
            >
          </div>
  ...
  >
    <div class="p-5 bg-white rounded dark:bg-darkBg">
      <div
        class="absolute top-4 left-5 cursor-pointer z-30"
        @click="visible = false"
      >
      ...
      <div class="pt-5 mt-6">
        ...
        >
          ...
          <NFormItem label="是否共享" path="public">
            <NSwitch
              v-model:value="appForm.public"
              :disabled="isAllowEditAppPublic"
            />
            <NTooltip placement="top-start" trigger="hover">
              <template #trigger>
                <SvgIcon
                  icon="ri:error-warning-line"
                  class="text-base ml-3 cursor-pointer"
                />
              </template>
              <p>Tips: 请知悉</p>
