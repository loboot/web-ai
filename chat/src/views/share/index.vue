...
import {
  fetchSalesAccountAPI,
  fetchSalesOrderAPI,
  fetchSalesRecordsAPI,
} from '@/api/sales';
...
const salesBaseRatio = computed(() =>
  globalConfig.value?.salesBaseRatio
    ? Number(globalConfig.value?.salesBaseRatio)
    : 10
);
const salesSeniorRatio = computed(() =>
  globalConfig.value?.salesSeniorRatio
    ? Number(globalConfig.value?.salesSeniorRatio)
    : 10
);
...
const inviteUrl = computed(() =>
  !inviteCode.value
    ? '**************'
    : `${domain.value}?inVitecode=${inviteCode.value}`
);
...
<template>
  <div class="main bg-[#f8f8fb] min-h-screen bg-center dark:bg-[#2F2E34]">
    ...
    />
    <div
      class="flex-1 flex-wrap py-5 flex justify-between"
      :class="[isMobile ? 'px-3' : 'px-20']"
    >
      <div
        class="px-[12px] min-w-[350px]"
      ...
      >
        ...
        >
          <div
            class="absolute right-4 top-6 font-bold text-base opacity-60 text-[#eee] flex"
          >
            ...
            <div class="flex flex-col">
              <div class="flex items-end">
                <span class="font-bold text-xl">{{
                  inviteAccount?.distributionBalance || 0
                }}</span>
              ...
              >
                <template #trigger>
                  <div
                    class="btn"
                    :class="[
                      inviteAccount?.distributionBalance <
                      Number(salesBaseRatio)
                        ? 'disabled'
                        : '',
                    ]"
                ...
                <span
                  >最低{{
                    Number(globalConfig?.salesAllowDrawMoney) || 10
                  }}元可提现!</span
                >
              </NPopover>
            </div>
        ...
        <!-- 数据统计 -->
        <div
          class="flex flex-col bg-[#fff] mt-5 rounded dark:bg-[#24272e] px-2"
        >
    ...
    <NModal :show="inviteDialog" style="width: 90%; max-width: 420px">
      <div class="p-4 bg-white rounded dark:bg-darkBg">
