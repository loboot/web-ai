...
const payPlatform = computed(() => {
  const { payHupiStatus, payEpayStatus, payMpayStatus, payWechatStatus } =
...
<template>
  ...
  >
    <div class="p-4 bg-white rounded dark:bg-darkBg">
      ...
      <div class="p-4">
        <div>
          <span class="whitespace-nowrap font-bold">需要支付：</span>
          <i class="text-xl text-[red] font-bold">{{
            `￥${orderInfo.pkgInfo?.price}`
          }}</i>
        </div>
        <div class="mt-2 flex">
          <span class="whitespace-nowrap font-bold">套餐名称：</span
          ><span class="ml-2"> {{ orderInfo.pkgInfo?.name }}</span>
        </div>
        <div class="mt-2 flex">
          <span class="whitespace-nowrap font-bold">套餐描述：</span
          ><span class="ml-2"> {{ orderInfo.pkgInfo?.des }} </span>
        </div>
        <!-- <div class="flex mt-3">
        ...
        >
          ...
          >
            ...
            <div
              ...
              class="mt-6 w-full text-center font-bold text-sm"
              :class="[isMobile ? 'mb-2' : 'mb-10']"
            >
              请在
              <span class="w-[60px] inline-block text-[red] text-left"
                ><NCountdown
                  ref="countdownRef"
                  :active="active"
                  :duration="300 * 1000"
