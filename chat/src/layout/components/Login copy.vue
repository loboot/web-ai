...
const disabledReg = computed(() => {
  return (
    ...
    !phoneRegisterStatus.value &&
    !emailRegisterStatus.value
  );
});
...
const registerTips = computed(
  () =>
    `首次认证：赠送${registerSendModel3Count.value}积分基础模型 | ${registerSendModel4Count.value}积分高级模型 | ${registerSendDrawMjCount.value}积分绘画`
);
...
<template>
  ...
  >
    <div class="p-5 bg-white rounded dark:bg-darkBg">
      ...
      <NAlert
        v-if="registerSendStatus && !disabledReg"
        type="success"
        :show-icon="false"
        class="mt-5"
      >
      ...
      <div v-if="disabledReg">
        ...
        >
          <template #footer>
            <NButton @click="authStore.setLoginDialog(false)"> 知道了 </NButton>
          </template>
        </NResult>
      </div>
    </div>
