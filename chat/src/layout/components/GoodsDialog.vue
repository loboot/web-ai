...
<template>
  <NModal
    :show="visible"
    :style="{
      maxWidth: `${packageList.length > 4 ? 1200 : packageList.length * 250}px`,
      minWidth: isSmallMd ? '100%' : '1000px',
    }"
    :on-after-enter="openDialog"
    :on-after-leave="handleCloseDialog"
  >
    <div class="p-4 bg-white rounded dark:bg-darkBg max-h-4/5">
      ...
      <div v-if="!loading" class="p-4">
        <NGrid
          ...
          :y-gap="15"
          :cols="
            isSmallMd ? 1 : packageList.length > 4 ? 4 : packageList.length
          "
          class="mt-3"
        >
            ...
            <NCard size="small" embedded>
              <template #header>
                <div class="relative">
                  <b>{{ item.name }}</b>
                  <img
                    v-if="item.extraReward === 1"
                    :src="preferentialIcon"
                    class="w-8 absolute -right-4 -top-3"
                  />
                </div>
              </template>
              <template #cover>
