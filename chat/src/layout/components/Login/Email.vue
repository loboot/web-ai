...
const isVerifyEmail = computed(() => {
  const v = authStore.globalConfig.isVerifyEmail
    ? Number(authStore.globalConfig.isVerifyEmail)
    : 1;
  return v;
});
...
const logTips = computed(() =>
  isLogin.value ? '还没账号? 去注册！' : '已有账户去登录！'
);
...
const emailRegisterStatus = computed(
  () => Number(authStore.globalConfig.emailRegisterStatus) === 1
);
  ...
  formRef.value?.validate(async (errors) => {
    if (!errors) {
      try {
        ...
        if (!isLogin.value) {
          const msg = Number(isVerifyEmail)
            ? '您的账号激活邮件已经发送,请前往邮箱激活您的账户！'
            : '您的账号已成功注册、请登录使用吧！';
...
<template>
  <div
    class="px-[65px] flex justify-center"
    :class="isLogin ? 'pt-[40px]' : 'pt-5'"
  >
    <NForm
      v-if="!isLogin"
      ref="formRef"
    ...
    >
      ...
      <Motion :delay="330">
        <NFormItem path="invitedBy">
          <NInput
            v-model:value="registerForm.invitedBy"
            placeholder="邀请码[非必填]"
          />
        </NFormItem>
      </Motion>
  ...
  <span class="flex justify-center cursor-pointer" v-if="emailRegisterStatus">
    <NButton text @click="isLogin = !isLogin">{{ logTips }}</NButton>
  </span>
  <div
    class="flex items-center justify-center space-x-5"
    :class="emailRegisterStatus ? 'mt-[16px]' : 'mt-[36px]'"
  >
    <n-button
    ...
    <n-button
      v-if="phoneLoginStatus"
      ghost
      class="!px-10"
      @click="emit('changeLoginType', 'phone')"
    >
      <SvgIcon
        class="text-xl mr-2 text-[#3076fd]"
