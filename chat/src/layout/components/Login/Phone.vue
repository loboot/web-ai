...
const rules: FormRules = {
  ...
  phoneCode: [{ required: true, message: '请填写手机验证码', trigger: 'blur' }],
};
...
const phoneRegisterStatus = computed(
  () => Number(authStore.globalConfig.phoneRegisterStatus) === 1
);
...
<template>
    ...
    >
      ...
      <Motion :delay="330">
        <NFormItem v-if="isSendCaptcha" path="phoneCode">
          <NInput
            v-model:value="registerForm.phoneCode"
            class="flex-1"
      ...
      <NFormItem>
        ...
        <div class="flex space-x-2 w-full" v-else>
          <NButton
            block
            type="primary"
            :disabled="loading"
          ...
          >
            重新发送{{
              lastSendPhoneCodeTime ? `(${lastSendPhoneCodeTime}S)` : ''
            }}
          </NButton>
        </div>
      </NFormItem>
  ...
  <span class="flex justify-center cursor-pointer" v-if="phoneRegisterStatus">
    <NButton text @click="isLogin = !isLogin">{{ logTips }}</NButton>
  </span>

  <div
    class="flex items-center justify-center space-x-5"
    :class="phoneRegisterStatus ? 'mt-[16px]' : 'mt-[36px]'"
  >
    <n-button
