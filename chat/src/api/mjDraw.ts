...
/* mj upscale Img */
export function fetchUpscaleSingleImgAPI<T>(data: {
  message_id: string;
  orderId: number;
}): Promise<T> {
  return post<T>({
...
/* mj variation img */
export function fetchVariationSingleImgAPI<T>(data: {
  message_id: string;
  orderId: number;
}): Promise<T> {
  return post<T>({
...
export type MjModeType = '' | 'mj-turbo' | 'mj-relax';

/* 提交一个绘画任务 */
export function fetchDrawTaskAPI<T>(data: {
  prompt?: string;
  imgUrl?: string;
  extraParam?: string;
  drawId?: number;
